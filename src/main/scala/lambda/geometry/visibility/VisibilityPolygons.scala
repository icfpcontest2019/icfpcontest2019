...
import lambda.geometry.{Direction, Point2D, PointPolar, Polygon, Ray2D, Segment}
...
object JoeSimpsonVisibility {
  ...
  def visibilityPolygon(pol: Polygon, p: Point2D): Option[Polygon] = {
  ...
  /**
    ...
    */
  private def visibilityInternal(pol: Polygon, z: Point2D): Polygon = {
  ...
  def advance(v: VsRep, s: MStack[VertDispl], iprev: Int): Seq[VertDispl] = {
    ...
    if (v(iprev + 1).alpha <=~ PI2) {
      ...
      if (s.top.alpha < PI2) {
        ...
        val isect = intersectSegmentWithRay(Segment(v(iprev), v(iprev + 1)), ray)
  ...
  def retard(v: VsRep, sold: MStack[VertDispl], iprev: Int): Seq[VertDispl] = {
    ...
    if (sj.alpha < v(iprev + 1).alpha) {
      ...
      val vi = v(i)
      val st1 = intersectSegmentWithRay(Segment(sj, sjnext), Ray2D(origin2D, vi.phi)).map {
  ...
  /**
    ...
    */
  def visibilityPrologue(z: Point2D, pol: Polygon): (VsRep, Double, Int) = {
  ...
  /**
    ...
    */
  private def visibilityEpilogue(pre_s: Seq[VertDispl], vs: VsRep, z: Point2D, initAngle: Double): Polygon = {
    ...
    val shiftedBack: Seq[Point2D] = rotatedBackSeq.map(_ + Direction(z.x, z.y))

    val res = Polygon(shiftedBack).removeAligned
...
object VisibilityUtil {
  ...
  implicit def _vd2Cart(v: VertDispl): Point2D = v.point.toCart
  ...
  /**
    ...
    */
  def getInitVertex(pol: Polygon, origIsVertex: Boolean): Point2D = {
    val es = pol.edges
    val vs = pol.vertices
  ...
  def locateSj(vi: VertDispl, vi1: VertDispl, sj1: VertDispl, ss: Seq[VertDispl]): (MStack[VertDispl], VertDispl) = {
    ...
    val y = intersectSegments(Segment(vi, vi1), Segment(sj, sj1))
