package lambda.geometry.visibility

import lambda.geometry.CommonUtils.GeometryException
import lambda.geometry.SegmentUtils._
import lambda.geometry.triangles.{Triangle, Triangulation}
import lambda.geometry.visibility.JoeSimpsonVisibility._
...
object VisibilityChecker {
  ...
  def toStringWithPoint(fr: FailReason, p: Point2D) = fr match {
  ...
  /**
    ...
    */
  def checkVisibility(pol: Polygon, points: Seq[Point2D]): (Boolean, Option[(Point2D, FailReason)], Set[Triangle]) = {
  ...
  private def splitTriangleByPolygons(ts: Set[Triangle], p: Polygon): Set[Triangle] =
  ...
  /**
    ...
    */
  def splitTriangleByPolygon(t: Triangle, vp: Polygon): Set[Triangle] = {
    ...
    def splitTriangleSet(ts: Set[Triangle], p: Point2D) = {
      ts.flatMap(t =>
  ...
  private def splitTriangleBySegments(ts: Set[Triangle], s: Segment): Set[Triangle] =
    ts.flatMap(splitTriangleBySegment(_, s))

  /**
    * Split the triangle, possibly intersected by the segment
    */
  def splitTriangleBySegment(t: Triangle, s: Segment): Set[Triangle] = {
  ...
  // Split affected triangles (progressibely)
  private def splitTriangleSet(ts: Set[Triangle], p: Point2D) = {
    ts.flatMap(t =>
      // p is on some edge of t, but not its vertex
      if (t.edges.exists(pointOnSegment(p, _)) && t.vertices.forall(_ =!= p))
        splitTriangleByEdgePoint(t, p)
  ...
  def splitTriangleByEdgePoint(t: Triangle, p: Point2D): Set[Triangle] = {
  ...
  def isInvisible(pol: Polygon, guards: Seq[Point2D], thief: Point2D) : (Boolean, String) = {
