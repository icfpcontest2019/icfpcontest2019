...
import lambda.geometry.{Polygon, SegmentUtils}
...
object DualGraphUtils {
  ...
  type NodeColoring = Map[Point2D, NodeColor]
  ...
  def constructColoring(p: Polygon): NodeColoring = {
  ...
  def chavatalVisibilitySet(p: Polygon): Seq[Point2D] = {
