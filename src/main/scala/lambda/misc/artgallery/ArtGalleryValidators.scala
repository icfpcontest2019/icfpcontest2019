...
import lambda.geometry.PolygonUtils._
import lambda.geometry.visibility.VisibilityChecker
import lambda.misc.artgallery.ArtGalleryInstance.{CheckSubmission, GuardSubmission}
import lambda.util.{ScenarioMessage, SolutionValidator}
import lambda.util.project.{BadSetupException, ProjectConstants, ScenarioInstance}
import ArtGalleryInstance._
...
class GuardsSolutionValidator(override val sRoot: String,
  ...
  type OkSolutionType = (Int, Seq[Point2D], Boolean, Int)
...
class CheckSolutionValidator(override val sRoot: String,
  ...
  type OkSolutionType = (Int, Point2D)
