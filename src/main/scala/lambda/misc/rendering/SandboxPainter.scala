...
import lambda.geometry.{Point2D, PointUtils, Polygon}
import lambda.geometry.generators.RandomCrazyPolygonGenerator
import lambda.geometry.triangles.DualGraphUtils
...
object HardPainter {
  ...
  def main(args: Array[String]) {
    val pol = Polygon(Seq((1, 1), (1, -3), (4, -3), (4, -1), (3, -1), (3, -2), (2, -2), (2, 0), (6, 0),
      (6, -1), (7, -1), (7, 1)).
        map { case v => PointUtils._point2D(v) })
...
//            val td = DualGraphUtils.chavatalVisibilitySet(pol)
    ...
    val td = Seq((7, -1), (1, 1), (4, -1)).map(PointUtils._point2D)
...
object AdaptCheckSolution {
  ...
  def main(args: Array[String]) {
    val pol = Seq().map(PointUtils._point2D)

    val td = Seq().map(PointUtils._point2D)
    val res = VisibilityChecker.checkVisibility(Polygon(pol), td)
    println(res)
    println(pol.size)
    println()
    drawShape(Polygon(pol), td)
    println(s"${Polygon(pol)}; ${Polygon(td.distinct)}")
  }
}
...
//    println()
//    println(td)
    val td = Seq((10.57, 9.4588), (12.0199, 8.20667), (1.4655, 12.4584), (2.74686, 0.673664), (3.44488, -9.24531), (7.67462, -3.07733), (5.76697, 14.1303), (1.79445, 8.02645), (3.78012, 18.08), (7.07661, 1.62677), (9.61021, -2.59891), (7.34606, -6.48815), (14.7262, -4.93806), (1.65211, 15.489), (-2.7689, -7.0445), (11.5371, -4.18502), (2.4363, 17.7584), (7.13772, -4.29319), (3.27763, 1.33134)).map { case v => PointUtils._point2D(v) }

    val res = VisibilityChecker.isInvisible(pol, td, Point2D(7.655472832078246, -1.1858709853435803))
    println(res)
...
//    val td = DualGraphUtils.chavatalVisibilitySet(pol)
    ...
    val td = Seq((3, 7.5), (8.02492235949962, 3.60557280900008)).map { case v => PointUtils._point2D(v) }
//    println(Polygon(td))
//    println()
    println(td.size)
...
object ExamplePainter {
  ...
  def main(args: Array[String]) {
    ...
    val pol = Polygon(Seq((0, 0), (1, 0), (1, -1), (-4, -1), (-4, 0), (-1, 0), (-1,
      ...
      (1, 2), (1, 1), (0, 1)).map { case (a, b) => Point2D(a, b) })

    val pol2 = Polygon(Seq((0, 0), (3, 0), (3, 1), (2, 1), (2, 2), (7, 2), (7, -2), (8, -2),
...
object ModelGuardsSolution {
  ...
  def generateAndCheckSolution(map: Map[Int, Polygon]): Unit = {
    for (k <- map.keySet.toSeq.sorted) {
      val pol: Polygon = map(k)
  ...
  def testExhausively(map: Map[Int, Polygon]): Unit = {
    for (k <- map.keySet.toSeq.sorted) {
      val pol: Polygon = map(k)
...
object UclLogoPainter {
  ...
  def main(args: Array[String]) {
    ...
    val pol = Polygon(s1.map(PointUtils._point2D))
...
object EnterprisePainter {
  ...
  def main(args: Array[String]) {
    val pol = Polygon(Seq((152, 26), (151, 20), (274, 16), (280, 21), (368, 28), (366, 35), (256, 43), (316, 135), (375, 135), (386, 128), (406, 130), (412, 125), (407, 117), (432, 85), (437, 86), (500, 63), (559, 60), (629, 78), (691, 114), (690, 211), (629, 246), (559, 262), (500, 264), (437, 238), (433, 239), (408, 208), (413, 199), (407, 196), (386, 196), (376, 190), (316, 187), (256, 281), (366, 289), (368, 295), (280, 302), (274, 308), (151, 315), (152, 301), (59, 295), (59, 289), (232, 282), (250, 229), (140, 225), (140, 219), (196, 215), (237, 179), (186, 172), (181, 165), (181, 161), (186, 154), (237, 146), (196, 108), (140, 105), (140, 98), (250, 96), (232, 43), (59, 34), (59, 29)).map(PointUtils._point2D))
    println(pol)
    println()
    val td = DualGraphUtils.chavatalVisibilitySet(pol)
    println()
