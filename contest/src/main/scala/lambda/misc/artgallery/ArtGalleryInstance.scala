...
object ArtGalleryInstance extends ScenarioInstance {
  ...
  def getGuardsPolygonMap(sDirPath: String): Map[Int, FPolygon] = {
  ...
  def retrieveInputForGuards(sDirPath: String, pNum: Int): Either[FPolygon, String] = {
    val polMap: Map[Int, FPolygon] = getGuardsPolygonMap(sDirPath)
  ...
  type Problem = (FPolygon, Seq[FPoint])
  ...
  def retrieveInputForCheck(sDirPath: String, pNum: Int): Either[Problem, String] = {
    val polMap: Map[Int, (FPolygon, Seq[FPoint])] = getProblemMap(sDirPath)
    if (polMap.contains(pNum)) Left(polMap(pNum))
    else Right(s"There is no polygon for the numer $pNum. Check your solution file.")
  }
}
