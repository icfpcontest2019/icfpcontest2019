...
import lambda.geometry.{GeometryParsers, floating}
...
object GuardInputParser extends GeometryParsers {
  def line: Parser[(Int, FPolygon)] = (wholeNumber ~ ":") ~ points(floatPoint) ^^ {
...
object GuardSolutionParser extends GeometryParsers {
  def line: Parser[(Int, Seq[FPoint])] = (wholeNumber <~ ":") ~ points(floatPoint) ^^ {
...
3: 5 / 16
 */
object GuardResultParser extends GeometryParsers {
 ...
 */
object CheckInputParser extends GeometryParsers {
  def line: Parser[(Int, (FPolygon, Seq[FPoint]))] = (wholeNumber <~ ":") ~
    points(floatPoint) ~ (";" ~> points(floatPoint)) ^^ {
...
object CheckSolutionParser extends GeometryParsers {
  def line: Parser[(Int, FPoint)] = (wholeNumber <~ ":") ~ floatPoint ^^ {
...
object CheckResultParser extends GeometryParsers {
