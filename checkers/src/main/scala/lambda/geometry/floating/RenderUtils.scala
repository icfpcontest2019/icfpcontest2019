...
object RenderUtils {
  ...
  def getPointsBoundingBox(points: Seq[FPoint]): (FPoint, FPoint) = {
    points.foldLeft((FPoint.MaxValue, FPoint.MinValue))(
      (candidate, box) => (candidate._1 glb box, candidate._2 lub box)
    )
  }

  // Gets the bounding box of a polygon.
  def getPolygonBoundingBox(polygon: FPolygon): (FPoint, FPoint) = {
    getPointsBoundingBox(polygon.vertices)
  }

  // Gets the bounding box of a sequence of polygons.
  def getPolygonBoundingBox(polygons: Seq[FPolygon]): (FPoint, FPoint) =
    polygons.foldLeft(FPoint.MaxValue, FPoint.MinValue)(
      (bound, polygon) => {
  ...
  def getSegmentBoundingBox(segment: FSegment): (FPoint, FPoint) =
    (segment._1 glb segment._2, segment._1 lub segment._2)

  // Gets the bounding box of a series of segments.
  def getSegmentBoundingBox(segments: Seq[FSegment]): (FPoint, FPoint) = {
    segments.foldLeft(FPoint.MaxValue, FPoint.MinValue)(
      (bound, segment) => {
  ...
  def getGraphicParameters(p: FPolygon) = {
  ...
  trait DrawingPanel extends JPanel {
    ...
    def drawComponents(graphics: Graphics): Unit = {
      ...
      var topLeft = (topLeftPoly glb topLeftPoint glb topLeftSegment).-(FPoint(1, 1))
      var bottomRight = (bottomRightPoly lub bottomRightPoint lub bottomRightSegment).-(FPoint(-1, -1))

      val xSpan = bottomRight.x - topLeft.x
      val ySpan = bottomRight.y - topLeft.y

      // Helpers that map a point to a coordinate of the canvas
      def mapX(point: FPoint) = ((point.x - topLeft.x) / xSpan * getWidth).toInt

      def mapY(point: FPoint) = ((point.y - topLeft.y) / ySpan * getHeight).toInt
    ...
    def polygonsToDraw(): Seq[(FPolygon, Color)]

    def polygonBordersToDraw(): Seq[(FPolygon, Color)]

    def segmentsToDraw(): Seq[(FSegment, Color)]

    def pointsToDraw(): Seq[(FPoint, Color)]
  }
