...
import lambda.geometry.visibility.VisibilityChecker._
import lambda.geometry.{Polygon, PolygonUtils}
...
object ArtGalleryPainter {
  ...
  def main(args: Array[String]): Unit = {
    ...
    assert(pol.containsPoint(Point2D(0, 0)))
    assert(pol.containsPoint(Point2D(1, 0)))
    assert(pol.containsPoint(Point2D(0, 1)))
    assert(pol.containsPoint(Point2D(1, 1)))
  ...
  case class PolygonProcessed(polygon: Polygon) {
    ...
    def processPoint(v: Point2D): (Int, Int) = {
    ...
    def processPoly(p: Polygon): (Seq[Int], Seq[Int], Int) = {
  ...
  def drawShape(p: Polygon, ps: Seq[Point2D], drawTriang: Boolean = false, drawCE: Boolean = true): Unit = {
  ...
  class PolyClass(pp: PolygonProcessed, ps: Seq[Point2D], drawTriang: Boolean, drawCE: Boolean) extends JPanel {
    ...
    def drawPoly(g: Graphics, p: Polygon, pp: PolygonProcessed, c: Color): Unit = {
      g.setColor(c)
      val vs = pp.processPoly(p)
      g.drawPolygon(vs._1.toArray, vs._2.toArray, vs._3)
    }

    def fillPoly(g: Graphics, p: Polygon, pp: PolygonProcessed, c: Color): Unit = {
      g.setColor(c)
      val vs = pp.processPoly(p)
      g.fillPolygon(vs._1.toArray, vs._2.toArray, vs._3)
    }

    def drawPoint(g: Graphics, pp: PolygonProcessed, p: Point2D, c: Color): Unit = {
      g.setColor(c)
